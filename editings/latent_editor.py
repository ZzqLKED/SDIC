...
class LatentEditor(object):
    ...
    def apply_ganspace(self, latent, ganspace_pca, edit_directions):
        ...
        images,G7 = self._latents_to_image(edit_latents)
        return images,G7, edit_latents
    ...
    def _latents_to_image(self, latents):
        with torch.no_grad():
            images,G7= self.generator([latents], None, randomize_noise=False, input_is_latent=True)
            if self.is_cars:
